...
class AppController {
    ...
    return _coreLifecycleLock.synchronized(() async {
      await safeRun(() async {
        ...
        if (configured) {
          ...
          await _updateGroups(preloadedProviders: providers);
          ...
          if (groups.isNotEmpty &&
            ...
            updateCurrentGroupName(groups.first.name);
          }
  ...
  Future<void> changeProxiesBatch(Map<String, String> groupToProxyMap) async {
    if (groupToProxyMap.isEmpty) return;

    for (final entry in groupToProxyMap.entries) {
      updateCurrentSelectedMap(entry.key, entry.value);
    }
    ...
    for (final entry in groupToProxyMap.entries) {
      try {
        ...
        commonPrint.log('Failed to change proxy for group ${entry.key}: $e');
      }
    }
    ...
    await updateGroups();
    addCheckIp();
    detectionState.startCheck(immediate: true, clearPrevious: true);
  }
  ...
  Future<void> changeMode(Mode mode) async {
    ...
    if (mode == Mode.global) {
      updateCurrentGroupName(GroupName.GLOBAL.name);
      if (hasDedicatedSelected && chainConfig.enable) {
        final target = chainConfig.createDedicatedGroup
            ? dedicatedGroupName
            : (activeLanding ?? '');
        if (target.isNotEmpty) {
          batchMap[GroupName.GLOBAL.name] = target;
        }
      }
    ...
    try {
      ...
      commonPrint.log('Failed to update clash config in changeMode: $e');
    }
